...
static RTIME t_period_ns = 100000;	/* timer period, in nanoseconds (.01 s)*/
...
int init_module(void)
{
  ...
  */
  retval = 
    rt_task_init(&p_task,	/* pointer to our task structure */
  ...
  if (safety==0) safety = 1;
   retval = 
    rt_task_make_periodic(&p_task, /* pointer to our task structure */
			  ...
			  timer_period_count*safety); /* recurring period As period = 0.01 s, the task will repeat every 0.1 seconds*/
  if (0 != retval) {
    if (-EINVAL == retval) {
      /* task structure is already in use */
