/*
  ...
  Sets up a task in pure periodic mode. In
...
static RT_TASK p_task;	/* we'll fill this in with our task */
static RT_TASK p_task2;
static RT_TASK p_task3;
static RTIME t_period_ns = 100000000;	/* timer period, in nanoseconds (.01 s)*/

static int which= 0;

/*
  t_function() is our task code, executed each period. RTAI requires
  all tasks to be of this form, taking an int argument and returning nothing.
  We'll ignore our argument.

  This simple task can be used to toggle the speaker port.
 */
void t_function(long arg)
{
  ...
  while (1) {
	...
	if (which==2) rt_printk("--------------------------------------Medium Priority task EXITS\n");
	if (which==3) rt_printk("--------------------------------------High Priority task EXITS\n");
	which = 1;
	
    }

    rt_printk("Low Priority task is LOCKED\n");

    rt_sleep(500000000);    
 
    rt_printk("Low Priority task is UNLOCKED\n");
...
void t_function2(long arg)
{
  ...
  while (1) {
	...
	if (which==3) rt_printk("--------------------------------------High Priority task EXITS\n");
	if (which==1) rt_printk("--------------------------------------Low Priority task EXITS\n");
	which = 2;
	
    }

    rt_printk("Medium Priority task is LOCKED\n");
    rt_sleep(100000000);
    rt_printk("Medium Priority task is UNLOCKED\n");
...
void t_function3(long arg)
{
  ...
  while (1) {
    ...
    rt_printk("High Priority task is LOCKED\n");
    rt_sleep(500000000);
    
    rt_printk("High Priority task is UNLOCKED\n");
...
rt_typed_sem_init(&sema,0,RES_SEM);
  ...
  */
  ...
  /*
    ...
    Use dmesg, or /var/log/messages, to see the rt_printk()
    messages.
  */
  r_period_count = nano2count(t_period_ns);
  timer_period_count = start_rt_timer(r_period_count);
  rt_printk("periodic_task: requested %d counts, got %d counts\n",
	    (int) r_period_count, (int) timer_period_count);
  ...
  /*
    ...
    state. It is made active either with
  ...
  */
  ...
  retval = 
    rt_task_init_cpuid(&p_task,	/* pointer to our task structure */
		 t_function, /* the actual timer function */
		 0,		/* initial task parameter; we ignore */
		 1024,		/* 1-K stack is enough for us */
		 3, /* lowest is fine for our 1 task */
		 0,		/* uses floating point; we don't */
		 0, 0);		/* signal handler; we don't use signals */

retval = 
    rt_task_init_cpuid(&p_task2,	/* pointer to our task structure */
		 t_function2, /* the actual timer function */
		 1,		/* initial task parameter; we ignore */
		 1024,		/* 1-K stack is enough for us */
		 2, /* lowest is fine for our 1 task */
		 0,		/* uses floating point; we don't */
		 0, 0);		/* signal handler; we don't use signals */

retval = 
    rt_task_init_cpuid(&p_task3,	/* pointer to our task structure */
		 t_function3, /* the actual timer function */
		 2,		/* initial task parameter; we ignore */
		 1024,		/* 1-K stack is enough for us */
   ...
   */
  retval = 
    rt_task_make_periodic(&p_task, /* pointer to our task structure */
  ...
  retval = 
    rt_task_make_periodic(&p_task2, /* pointer to our task structure */
			  /* start one cycle from now */
			  rt_get_time() + timer_period_count, 
			  7*timer_period_count); /* recurring period As period = 0.01 s, the task will repeat every 0.1 seconds*/


  retval = 
    rt_task_make_periodic(&p_task3, /* pointer to our task structure */
			  /* start one cycle from now */
			  rt_get_time() + timer_period_count, 
			  20*timer_period_count); /* recurring period As period = 0.01 s, the task will repeat every 0.1 seconds*/
...
//rt_spv_RMS(0);
  if (0 != retval) {
    ...
    //rt_task_resume(&p_task);

    //rt_task_resume(&p_task2);

    //rt_task_resume(&p_task3);
...
void cleanup_module(void)
{
  ...
  retval = rt_task_delete(&p_task);
  retval = rt_task_delete(&p_task2);
  retval = rt_task_delete(&p_task3);
